...
public class ScrambleSolver{
	...
	private static Word[] solve(String plainText, String[] words, int max){
	 System.out.println("Words: ");
	 for(String a : plainText.split(" ")){
		 System.out.print(a+" ");
	 }
	 System.out.println(" ");
	 ...
	 for(int x=0;x<words.length;x++){
	  if(checkIfAllMaxTrue(returnThisObject,max)){
		...
		for(Word oneWord : returnThisObject){
			...
			if(checkForAnagram(oneWord.id,words[x])){
				oneWord.possible.add(words[x]);
			}
		  }
		}
	  }
